...
temp_db_collection = db.twitter_search3.find().limit(245)
...
for data in store_ascii_tweets:
    ...
    if(data["user"]["geo_enabled"]==True and data["place"] is not None):
        #Assign Glasgow as location to half of the tweets
        #place = "Glasgow"
...
#ax1.set_xlabel("Group of Tweets with assigned location")
#ax1.set_ylabel("Total tweets in each group")
#ax1.set_title("Number of tweets in each group when a location is assigned")
ax1.set_xlabel("Group of Tweets with actual location")
ax1.set_ylabel("Total tweets in each group")
ax1.set_title("Number of tweets in each group when actual location is concerned")
for key,value in groups.items():
    x[i]=i
